...
update_installer() {
    ...
    # Update the flake.nix generation to use configuration/variants/configuration-minimal.nix
    sed -i 's|(hypervisor + "/configuration.nix")|(hypervisor + "/configuration/variants/configuration-minimal.nix")|' "$installer"
    
    echo "✓ Updated installer to use minimal configuration"
}

# Update configuration/variants/configuration-minimal.nix to import first-boot module
update_minimal_config() {
    local minimal_config="configuration/variants/configuration-minimal.nix"
...
- Restore backup: `sudo cp /etc/nixos/configuration/configuration.nix.backup.* /etc/nixos/configuration/configuration.nix`
